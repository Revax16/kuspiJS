...
KUSPI.ParticleEmitter = class {
    constructor (set) {
        ...
        this.p = {
            ...
            texture:set.particles.texture || undefined,
            color:{
                cur:0,
                col:set.particles.color || ['AAA', 'BBB', 'CCC', 'DDD', 'EEE']
            },
            perFrame:set.particles.perFrame || 5,
    ...
    newParticles () {
        ...
        for (var i = 0; i < amount; i++) {
            p.color.cur++;
            p.total++;
            if (p.color.cur >= p.color.col.length) p.color.cur = 0;
            this.particles.push(new KUSPI.Particle({
                p:this,
                width:p.width,
                height:p.height,
                duration:p.duration,
                color:p.color.col[p.color.cur],
                texture:p.texture,
            }));
        }
    }
    ...
    remove (uid) {
        for (var i in this.particles) {
            if (this.particles[i].uid === uid) { this.particles.splice(i, 1); this.p.total--; }
        }
    }
